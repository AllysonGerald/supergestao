...
<div class="space-y-6">
    ...
    <form action="{{ route('admin.produtos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">@csrf
        <div class="bg-white rounded-xl shadow-lg p-6"><h2 class="text-xl font-bold mb-6">Dados do Produto</h2><div class="grid md:grid-cols-2 gap-6">
            ...
            <div><label class="block text-sm font-medium mb-2">Preço Custo *</label><input type="text" name="preco_custo" required data-mask="currency" placeholder="R$ 0,00" class="w-full px-4 py-2 border rounded-lg"></div>
            <div><label class="block text-sm font-medium mb-2">Preço Venda *</label><input type="text" name="preco_venda" required data-mask="currency" placeholder="R$ 0,00" class="w-full px-4 py-2 border rounded-lg"></div>
