...
<div class="space-y-6">
    ...
    <form action="{{ route('admin.produtos.update', $produto) }}" method="POST" enctype="multipart/form-data" class="space-y-6">@csrf @method('PUT')
        <div class="bg-white rounded-xl shadow-lg p-6"><h2 class="text-xl font-bold mb-6">Dados do Produto</h2><div class="grid md:grid-cols-2 gap-6">
            ...
            <div><label class="block text-sm font-medium mb-2">Preço Custo *</label><input type="text" name="preco_custo" value="{{ number_format($produto->preco_custo, 2, ',', '.') }}" required data-mask="currency" placeholder="R$ 0,00" class="w-full px-4 py-2 border rounded-lg"></div>
            <div><label class="block text-sm font-medium mb-2">Preço Venda *</label><input type="text" name="preco_venda" value="{{ number_format($produto->preco_venda, 2, ',', '.') }}" required data-mask="currency" placeholder="R$ 0,00" class="w-full px-4 py-2 border rounded-lg"></div>
