...
<div class="space-y-6">
    ...
    <form action="{{ route('admin.pedidos.update', $pedido) }}" method="POST" class="space-y-6">@csrf @method('PUT')
        <div class="bg-white rounded-xl shadow-lg p-6"><h2 class="text-xl font-bold mb-6">Dados do Pedido</h2><div class="grid md:grid-cols-2 gap-6">
            ...
            <div><label class="block text-sm font-medium mb-2">Desconto</label><input type="text" name="desconto" value="{{ number_format($pedido->desconto, 2, ',', '.') }}" data-mask="currency" placeholder="R$ 0,00" class="w-full px-4 py-2 border rounded-lg"></div>
